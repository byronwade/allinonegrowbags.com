...
export default function ShopifyCheckout() {
	const { quantity, getCurrentPrice, getDiscount, basePrice, compareAtPrice, isLoading } = useCart();
			...
			const response = await fetch("/api/cart", {
				...
				body: JSON.stringify({
					...
					items: [
						{
							quantity: quantity,
							price: basePrice, // Original price from Shopify
	...
	const discount = getDiscount(quantity);
	const savings = discount > 0 ? basePrice * quantity - totalPrice : 0;
	...
	return (
		<div className="space-y-4" suppressHydrationWarning>
								...
								{discount > 0 && (
									<>
										...
										<div className="text-xs text-gray-400">
											<s>${basePrice.toFixed(2)}</s> original price
										</div>
									</>
								)}
							</>
			...
			<Button className="w-full bg-purple hover:bg-purple-dark text-white py-6 text-lg font-semibold" onClick={handleBuyNow} disabled={isSubmitting || isLoading}>
				{isSubmitting ? "Adding to Cart..." : `Buy Now - $${totalPrice.toFixed(2)}`}
			</Button>
